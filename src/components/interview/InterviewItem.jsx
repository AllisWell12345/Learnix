...
import { addComment, deleteComment, updateComment } from "../../store/interviewSlice";
...
function InterviewItem({ interview, currentUser }) {
    ...
    <div className="interviewitem-wrap">
      ...
      <div className="interviewitem-info-row">
        ...
        <div className="interviewitem-card interviewitem-project-card">
          ...
          <div className="interviewitem-info-field">
            ...
            <p className="interviewitem-info-val interviewitem-info-bold">{interview.projectTitle}</p>
          </div>
          <div className="interviewitem-info-field">
        ...
        <div className="interviewitem-card interviewitem-student-card">
          ...
          <div className="interviewitem-info-field">
            ...
            <p className="interviewitem-info-val interviewitem-info-bold">{interview.student.name}</p>
          </div>
          <div className="interviewitem-info-field">
            <span className="interviewitem-info-label">이메일</span>
            <p className="interviewitem-info-val">{interview.student.email}</p>
          </div>
          <div className="interviewitem-info-field">
        ...
        <div className="interviewitem-qa-list">
          {interview.questions.map((qa, index) => (
            <div className="interviewitem-qa-item" key={index}>
              <p className="interviewitem-question">질문 {index + 1}. {qa.question}</p>
      ...
      <div className="interviewitem-card">
        <div className="interviewitem-comment-header">
          ...
          <span className="interviewitem-comment-count">({interview.comments.length})</span>
        </div>
        ...
        <div className="interviewitem-comment-input-row">
          ...
          />
          <button className="interviewitem-comment-submit-btn" onClick={handleAddComment}>
            ...
            >
              <div className="interviewitem-comment-top">
                <div className="interviewitem-comment-author-area">
                  <span className="interviewitem-comment-author">
                    {comment.author}
                    {comment.role === "teacher" && <span className="interviewitem-teacher-badge">(나)</span>}
                  </span>
                  <span className="interviewitem-comment-date">{comment.date}</span>
                </div>
                <div className="interviewitem-comment-actions">
                  {/* 강사 본인 댓글이면 수정/삭제 */}
                  {isTeacher && comment.role === "teacher" && (
                    <>
                      <button className="interviewitem-edit-btn" onClick={() => handleEditStart(comment)}>수정</button>
                      <button className="interviewitem-delete-btn" onClick={() => handleDelete(comment.commentId)}>삭제</button>
                    </>
                  )}
                  {/* 본인이 아닌 댓글엔 신고 */}
                  {!(isTeacher && comment.role === "teacher") && (
              ...
              {editId === comment.commentId ? (
                <div className="interviewitem-edit-row">
                  ...
                  />
                  <button className="interviewitem-edit-done-btn" onClick={() => handleEditDone(comment.commentId)}>완료</button>
                </div>
              ) : (
                <p className="interviewitem-comment-content">{comment.content}</p>
              )}
            </div>
          ))}
        </div>
